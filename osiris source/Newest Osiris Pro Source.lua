--// 1 \\-- 
-- Services
local InputService, TeleportService, RunService, Workspace, Lighting, Players, HttpService, StarterGui, ReplicatedStorage  = game:GetService("UserInputService"), game:GetService("TeleportService"), game:GetService("RunService"), game:GetService("Workspace"), game:GetService("Lighting"), game:GetService("Players"), game:GetService("HttpService"), game:GetService("StarterGui"), game:GetService("ReplicatedStorage")
-- Decendants 
local LocalPlayer = Players.LocalPlayer
local Wanted = LocalPlayer:WaitForChild("DataFolder").Information.Wanted
local OldWanted = Wanted.Value
local Remote = ReplicatedStorage.MainEvent
local Mouse, Camera, Dummy = LocalPlayer:GetMouse(), Workspace.Camera, game:GetObjects("rbxassetid://9474737816")[1]
local Find, Clear, Sub, Upper, Lower = table.find, table.clear, string.sub, string.upper, string.lower
...
-- Variables 
local Osiris = {
    ["shit talk"] = {["Osiris"]={"sorry guys 😭 I pasted stormy 😔","Injecting ChatGPT.Hook!","Blah Blah Blah 😭😭😭😭","Clipped! -🤓","Sorry guys 🥺 Im using 0 prediction resolver😢","farthook ontop!!!!","Osiris 💳💥💳💥💳💥💳💥","Farthook Successfully Injected!","Guys Im using jjsploit!!!","bwootooth dwevice rwaeady to pwear 🤖🤖🤖","Yun is the best script -🤖","Buy the script at gg/osr","🍑👅❓","mafafaka 😡","stormy.solutions is the best da hood script!!!","Bluetooth bullets: Enabled 🤖"},["Yun"]={"YUN RUNS YOU"},["Specter"]={"specter.lua > u","sonned?","sad life","u shall quit"},["Anti Aim"]={"Cant resolve me!? Cant resolve me!? Cant resolve me!? Cant resolve me!? Cant resolve me!?"},["Advertisement"]={"OSIRIS THE BEST BUY OSIRIS AT GG/OSR","GOOGOO GAH GAH NO OSIRIS 😭😭"},["Scottish"]={"You Grandma Still Wears Shin Pads To Work 🤣🤣","Melon Head","Your Ma Is A Bin Man 🤣🤣","Your Da Sells Osiris 🤣🤣","Even Osiris Is Better Than You XD","Taped You Like I Did To Your Ma","Fore Headed Mong","Such A Fruit","YoUr A BoOt","keep Trying You Jobby"}},
    ["sfx"] = {["Bameware"]="3124331820",["Skeet"]="4753603610",["Bonk"]="3765689841",["Lazer Beam"]="130791043",["Windows XP Error"]="160715357",["TF2 Hitsound"]="3455144981",["TF2 Critical"]="296102734",["TF2 Bat"]="3333907347",['Bow Hit']="1053296915",['Bow']="3442683707",['OSU']="7147454322",['Minecraft Hit']="4018616850",['Steve']="5869422451",['1nn']="7349055654",['Rust']="3744371091",["TF2 Pan"]="3431749479",["Neverlose"]="8679627751",["Mario"]="5709456554"},
...
for i,v in pairs(LocalPlayer.Character:GetChildren()) do 
    if v:IsA("BasePart") then 
        table.insert(Osiris.bodyParts,v.Name)
    end 
end 

for i,v in next, Osiris.Themes do 
    table.insert(Osiris.themeNames,i)
end 

for i,v in next, Osiris.sfx do 
    table.insert(Osiris.sfxnames,i)
end 
...
--// Functions 
    ...
    --
    function Osiris:ValidateParts(Player)
    ...
    --
    function Osiris:GetClosestPlayer()
        local shortestDistance = Huge
    
        local closestPlayer
        for _, Player in pairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer and Osiris:GetPlayerStatus(Player) then
            --
            ...
            for i, v in pairs(Player.Character:GetChildren()) do
                if v:IsA("BasePart") then 
                    ...
                    local magnitude = (Vector2.new(pos.X, pos.Y) - Vector2.new(Mouse.X, Mouse.Y+36)).magnitude
                        if magnitude < shortestDistance and Find(List,v.Name) then
                            closestPart = v
                            shortestDistance = magnitude
                        end
                    end
    ...
    -- 
    function Osiris:CheckVisibility(char, part) -- ## Target.Character, "HumanoidRootPart"
        return not unpack(Camera:GetPartsObscuringTarget({LocalPlayer.Character[part].Position, char[part].Position}, {LocalPlayer.Character, char}))
    end
    -- 
    function Osiris:CloneCharacter(Player, Color, Material, Transparency)
        for i,v in pairs(Player.Character:GetChildren()) do 
    ...
    function Osiris:Unload()
        for _, Connection in pairs(Osiris.Connections) do
    ...
    function Osiris:CharcterInFov(Player, FOV) 
        if FOV.Visible == false then return true end 

        local FOVCheck = FOV.Radius
        ...
        spawn(function()
            ...
            for i,v in pairs(Explosion:GetChildren()) do
                if v:IsA('Part') then
                    ...
                    v.CFrame = Explosion.CFrame * RandomOffsets[i][Random(1, #RandomOffsets[i])]
                end
            end
    ...
    function AirStrike(VictimChar)
        spawn(function()
            ...
            local function AddExplosion()
                ...
                Explosion.Position = VictimChar.UpperTorso.Position + Vector3.new(Random(0,50) * 0.1, 0, Random(0,50) * 0.1)
                ExplosionSound:Play()
            end
            task.spawn(function()
    ...
    function CosmicSlash(VictimChar)
        spawn(function()
...
--// Target Stats        
    ...
    for Index, Value in pairs(Options) do
        ...
        Title.Size = 13
        Title.Font = 2
        Title.Text = "".. LocalPlayer.Name .." [" .. LocalPlayer.DisplayName .."]"
...
--// Library
    local library, pointers = loadstring(game:HttpGet("https://hvh.wtf/p/raw/osiris/w06h1wszhq"))()
    --local library, pointers = loadfile("gz.txt")()
    ...
    do
            ...
            lock:Toggle({name = "Enabled", pointer = 'lock_enabled'}):Keybind({name = "Locking", pointer = 'lock_key', mode = "Toggle",callback = function(a) 
                if pointers["lock_enabled"]:get() and pointers["lock_type"]:get() == "Lock" and not Holding then 
                    if pointers["lock_key"]:is_active() then 
                        getgenv().Target = getClosestPlayerToCursor(Huge) 
                        ...
                        TargetPart.CFrame = CFrame.new(9999,9999,9999)
                        CC.CameraSubject = LocalPlayer.Character.Humanoid
        ...
        do 
            aim_assist:Toggle({name = "Enabled", pointer ='aim_assist_enabled'}):Keybind({name = "Aim Assist", keybind_name = "Aim Assist", pointer ='aim_assist_key', mode = "Toggle",callback = function(a)
                if pointers["aim_assist_enabled"]:get() and pointers["aim_assist_key"]:is_active() then 
                    getgenv().CTarget = getClosestPlayerToCursor(Huge)
                else 
                    getgenv().CTarget = nil 
                end 
            end})
        ...
        do 
            ...
            function WallCheck(head)
                if not Find(pointers["checks"]:get(), "Wall Check") then 
                    return false 
                end 
                ...
                local ignoreList = {LocalPlayer.Character,head.Parent,ws.Terrain}
            ...
            function KnockCheck(head)
                if not Find(pointers["checks"]:get(), "Knock Check") then return true end 
        ...
        do 
            ...
            Part_Section:Dropdown({name = "configs", pointer = 'part_configs', options = {'custom','hitbox',"bubble"}, default = 'custom',callback = function(a)
                if pointers["part_configs"]:get() == "hitbox" then 
                    TargetPart.Size = Vector3.new(LocalPlayer.Character.HumanoidRootPart.Size.X * 3 ,5,LocalPlayer.Character.HumanoidRootPart.Size.X * 3)
    ...
    do 
        ...
        local cframe_section,velocity_section,network_section               = misc_page:MultiSection({sections = {"Client","Anti-Lock","Network"}, side = "Right", Size = 450,fill = true}) 
        ...
        do 
            ...
            disablers_section:Toggle({pointer = "remove_zoom_limit", name = "Remove Zoom Limit", def = false,callback = function()
                if pointers["remove_zoom_limit"]:get() then 
                    LocalPlayer.CameraMaxZoomDistance = Huge
                else 
            ...
            {{"Buy Gun", function() 
                if pointers["Auto-Buy"]:get() then 
                    ...
                    LocalPlayer.Character.HumanoidRootPart.CFrame = ws.Ignored.Shop[pointers["Auto-Buy"]:get()].Head.CFrame
                    wait(0.20)
            ...
            , {"Buy Ammo", function() 
                for i = pointers["Ammo_Buy_Amount"]:get(),0,-1 do 
                    ...
                    if pointers["Auto-Ammo"]:get() then 
                        local OldPosition = LocalPlayer.Character.HumanoidRootPart.CFrame
                        LocalPlayer.Character.HumanoidRootPart.CFrame = ws.Ignored.Shop[pointers["Auto-Ammo"]:get()].Head.CFrame
                        wait(0.25)
            ...
            {{"Buy Food", function() 
                if pointers["Auto-Food"]:get() then 
                    local OldPosition = LocalPlayer.Character.HumanoidRootPart.CFrame
                    LocalPlayer.Character.HumanoidRootPart.CFrame = ws.Ignored.Shop[pointers["Auto-Food"]:get()].Head.CFrame
                    wait(0.5)
            ...
            , {"Buy Armor", function() 
                    local OldPosition = LocalPlayer.Character.HumanoidRootPart.CFrame
                    LocalPlayer.Character.HumanoidRootPart.CFrame = ws.Ignored.Shop["[High-Medium Armor] - $2163"].Head.CFrame
                    wait(0.5)
        ...
        do 
            ...
            cframe_section:Toggle({name = "Enabled", pointer ='cframe_desync',callback = function(a)                    
                Visualizecframe.HumanoidRootPart.CFrame = CFrame.new(9999,9999,9999)
                if pointers["body_weld_visualization"]:get() then 
            ...
            end}):Keybind({name = "Desync Key", pointer ='cframe_desync_key', mode = "Toggle",default = Enum.KeyCode.X, callback = function(Bool) 
                Visualizecframe.HumanoidRootPart.CFrame = CFrame.new(9999,9999,9999)
                ...
                if Bool then 
                    ...
                    LocalPlayer.Character.HumanoidRootPart.CFrame = getgenv().OldCFrame
                end 
            end})
            ...
            cframe_section:Dropdown({name = "Material", pointer ='visualize_material', options = {'ForceField','Neon',"Plastic"}, default = 'ForceField',callback = function(a)
                for i,v in pairs(Visualizecframe:GetChildren()) do 
                    if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" then 
                        ...
                        v.Transparency = pointers["visualize_cframe_color"]:get().Transparency
                    end 
                end 
            end})
            cframe_section:Toggle({name = "Visualize Absolute Position", pointer = 'body_weld_visualization',callback = function(Bool)
                if Bool then 
                    Visualizecframe.HumanoidRootPart.CFrame = CFrame.new(9999,9999,9999)
                else 
                    LocalPlayer.Character.LowerTorso.Root.Part0 = LocalPlayer.Character.HumanoidRootPart
            ...
            task.spawn(function()
                while true do 
                    ...
                    if pointers["st"]:get() then 
                        local RandomChat = Random(1,#client["shit talk"][tostring(pointers["st_type"]:get())])
    ...
    do 
        ...
        do
            ...
            Player_List_Left:Button({name = "Teleport To", confirmation = false, callback = function()
                LocalPlayer.Character.HumanoidRootPart.CFrame = game.Players[tostring(Player_List:GetSelection()[1])].Character.HumanoidRootPart.CFrame 
            end})
            -- 
...
--// Code
    task.spawn(LPH_NO_VIRTUALIZE(function()
        while Run.RenderStepped:Wait() do 
            ...
            if LocalPlayer.Character:FindFirstChildWhichIsA("Tool") ~= nil and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"):FindFirstChild("Handle") ~= nil and pointers["custom_gun_sfx"]:get() then 
                ...
                if tool() and pointers["bullet_tracers"]:get() and tool():FindFirstChild("Ammo") then  
                    getgenv().Loop = tool().Ammo.Changed:Connect(function()
                        if tool().Ammo.Value ~= tool().MaxAmmo.Value then 
                            createBeam(tool().Handle.Position, LocalPlayer.Character.BodyEffects.MousePos.Value) 
    ...
    local function Aimbot()
        if pointers["aim_viewer_bypass"]:get() then 
    ...
    for i, v in pairs(LocalPlayer.Backpack:GetChildren()) do
        if v:IsA("Tool") and not client.connections["anti aim viewer"][v] then
            client.connections["anti aim viewer"][v] = v.Activated:Connect(Aimbot)
        end
    end
    for i, v in pairs(LocalPlayer.Character:GetChildren()) do
        if v:IsA("Tool") and not client.connections["anti aim viewer"][v] then
    ...
    Run:BindToRenderStep("CFrame Desync", 1, LPH_NO_VIRTUALIZE(function()
        if pointers["cframe_desync"]:get() and pointers["cframe_desync_key"]:is_active() and LocalPlayer.Character then
            getgenv().OldCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame 
            local Sine_Value = abs(sin(tick() * (pointers["sine_speed"]:get()/5)))
            local CFramePositions = {
                ["Random"] = (CFrame.new(LocalPlayer.Character.HumanoidRootPart.Position) + Vector3.new(Random(-pointers["random_range"]:get(), pointers["random_range"]:get()), Random(-pointers["random_range"]:get(), pointers["random_range"]:get()), Random(-pointers["random_range"]:get(), pointers["random_range"]:get()))) * CFrame.Angles(rad(Random(-180, 180)), rad(Random(-180, 180)), rad(Random(-180, 180))),
                ["Upside Down"] = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(0,0,rad(-180)),
                ["Floor Plant"] = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0,-4,0) * CFrame.Angles(0,rad(Random(1,360)),rad(-180)),
                ["Sine"] = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(rad(Sine_Value*360),rad(Sine_Value*360),rad(Sine_Value*360)) * CFrame.Angles(0,0,math.rad(-180))
            }

            local CFrameVisualizations = {
                ["Random"] = CFramePositions.Random,
                ["Upside Down"] = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(0,0,rad(-180)),
                ["Floor Plant"] = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0,-4,0) * CFrame.Angles(0,rad(Random(1,360)),rad(-180)),
                ["Sine"] = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(math.rad(Sine_Value*360),math.rad(Sine_Value*360),math.rad(Sine_Value*360))
            }

        
            if pointers["custom_cframe"]:get() then
                if pointers["CFrame_Type"]:get() ~= "Nothing" then 
                    LocalPlayer.Character.HumanoidRootPart.CFrame = CFramePositions[pointers["CFrame_Type"]:get()] * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
                else 
                    LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
                end 
            elseif pointers["CFrame_Type"]:get() ~= "Nothing" and pointers["custom_cframe"]:get() then 
                LocalPlayer.Character.HumanoidRootPart.CFrame = CFramePositions[pointers["CFrame_Type"]:get()] * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
            end 

            if pointers["visualize_cframe"]:get() and not pointers["body_weld_visualization"]:get() and pointers["CFrame_Type"]:get() ~= "Nothing" then 
                if pointers["custom_cframe"]:get() then 
                    Visualizecframe.HumanoidRootPart.CFrame = CFrameVisualizations[pointers["CFrame_Type"]:get()]
                else 
                    Visualizecframe.HumanoidRootPart.CFrame = CFrameVisualizations[pointers["CFrame_Type"]:get()] * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
                end 
            elseif pointers["visualize_cframe"]:get() and pointers["custom_cframe"]:get() and pointers["CFrame_Type"]:get() == "Nothing" and not pointers["body_weld_visualization"]:get() then 
                Visualizecframe.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
            end
            
            

            if pointers["body_weld_visualization"]:get() then 
                if pointers["custom_cframe"]:get() and pointers["CFrame_Type"]:get() == "Nothing" then 
                    for i,v in pairs(LocalPlayer.Character:GetChildren()) do 
                        if v:IsA("BasePart") then 
                            v.CanCollide = false 
                        end 
                    ...
                    LocalPlayer.Character.LowerTorso.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0,-0.8,0) * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
                    LocalPlayer.Character.LowerTorso.Anchored = true   
                elseif pointers["CFrame_Type"]:get() ~= "Nothing" then 
                    if pointers["custom_cframe"]:get() then 
                        for i,v in pairs(LocalPlayer.Character:GetChildren()) do 
                            if v:IsA("BasePart") then 
                                v.CanCollide = false 
                        ...
                        LocalPlayer.Character.LowerTorso.Root.Part0 = LocalPlayer.Character.LowerTorso
                        LocalPlayer.Character.LowerTorso.CFrame = CFrameVisualizations[pointers["CFrame_Type"]:get()] * CFrame.new(pointers["CFrame_X"]:get(),pointers["CFrame_Y"]:get(),pointers["CFrame_Z"]:get()) * CFrame.Angles(rad(pointers["pitch"]:get()),rad(pointers["yaw"]:get()),rad(pointers["roll"]:get()))
                        LocalPlayer.Character.LowerTorso.Anchored = true   
                        print("elo")
                    else 
                        for i,v in pairs(LocalPlayer.Character:GetChildren()) do 
                        ...
                        LocalPlayer.Character.LowerTorso.Root.Part0 = LocalPlayer.Character.LowerTorso
                        LocalPlayer.Character.LowerTorso.CFrame = CFrameVisualizations[pointers["CFrame_Type"]:get()]
                        LocalPlayer.Character.LowerTorso.Anchored = true   
                    end 
                end 
    ...
    Run:BindToRenderStep("Main", 1, LPH_NO_VIRTUALIZE(function(FPS)
        ...
        --
        if Target ~= nil and pointers["lock_enabled"]:get() and pointers["lock_key"]:is_active() and Target and Target.Character and Target.Character.Humanoid ~= nil and Target.Character.Humanoid.Health > 0 and KnockCheck(Target) and pointers["target_stats"]:get() then 
            local Z = floor(Target.Character.Humanoid.Health)
            Sliders["Health"]:Set(Z / 100, (floor(Z / 0.01) * 0.01) .. "")
        end 
        --
        ...
        --
        if pointers["velocity_desync"]:get() and LocalPlayer.Character then   
            local DesyncVelocitys = {
                ...
                ["Follow Camera"] = (Mouse.hit.p - LocalPlayer.Character.HumanoidRootPart.Position).Unit * (pointers["follow_camera_power"]:get()*5) + Camera.CFrame.LookVector * (pointers["follow_camera_power"]:get()*5),
                ["Prediction Multiplier"] = LocalPlayer.Character.HumanoidRootPart.Velocity * pointers["prediction_multiplier_power"]:get(),
            ...
            if pointers["velocity_desync_key"]:is_active() then 
                LocalPlayer.Character.HumanoidRootPart.Velocity = DesyncVelocitys[pointers["velocity_type"]:get()]
                LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(0,rad(0.001),0)                    
                getgenv().Visualizer = LocalPlayer.Character.HumanoidRootPart.Velocity
                if pointers["visualize_velocity"]:get() then 
                    if getgenv().Visualizer ~= nil then 
                        Visualizevelocity.HumanoidRootPart.CFrame = CFrame.new(LocalPlayer.Character.HumanoidRootPart.Position + getgenv().Visualizer * 0.1413)
                    end 
                end 
        ...
        --
        if pointers["anti_slow_down"]:get() and LocalPlayer.Character then 
        ...
        --
        if pointers["anti_jump_cooldown"]:get() and LocalPlayer.Character then 
        ...
        if pointers["auto_reload"]:get() then 
            if LocalPlayer.Character:FindFirstChildWhichIsA("Tool") ~= nil then
                if LocalPlayer.Character:FindFirstChildWhichIsA("Tool"):FindFirstChild("Ammo") then
                    if LocalPlayer.Character:FindFirstChildWhichIsA("Tool"):FindFirstChild("Ammo").Value <= 0 and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"):FindFirstChild("Ammo").Value == 0  then
        ...
        --  
        if pointers["speed"]:get() and pointers["speed_key"]:is_active() and LocalPlayer.Character and not Holding then 
            LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame + LocalPlayer.Character.Humanoid.MoveDirection * (pointers["speed_power"]:get()/25)
            if pointers["auto_jump"]:get() and LocalPlayer.Character.Humanoid.FloorMaterial ~= Enum.Material.Air then 
        ...
        if pointers["spin"]:get() and pointers["spin_key"]:is_active() then 
            LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(0,rad(pointers["spin_power"]:get()),0)
        ...
        if pointers["fly"]:get() and pointers["fly_key"]:is_active() and not Holding then 
            ...
            if FlyPosition.Unit.y == FlyPosition.Unit.y then
                LocalPlayer.Character.HumanoidRootPart.Anchored = false 
                LocalPlayer.Character.HumanoidRootPart.Velocity = FlyPosition.Unit * pointers["fly_speed"]:get()
            else 
                LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
        ...
        if pointers["jitter"]:get() and pointers["jitter_key"]:is_active() then 
            ...
            LocalPlayer.Character.Humanoid.AutoRotate = false
            LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(LocalPlayer.Character.HumanoidRootPart.CFrame.Position) * CFrame.Angles(0, rad(RandomJit) + rad((random(1, 2) == 1 and RandomJit or -RandomJit)), 0) 
        end 
        --
        if Target ~= nil and pointers["lock_enabled"]:get() and pointers["lock_key"]:is_active() and Target.Character and Target.Character.Humanoid and KnockCheck(Target) then 
            ...
            if pointers["look_at"]:get() and LocalPlayer.Character then 
                LocalPlayer.Character.Humanoid.AutoRotate = false 
            ...
            if tonumber(Wanted.Value) == tonumber(OldWanted + 50) then
                task.spawn(function()
                    if Stomping == true then
                        local Part = workspace:FindPartOnRayWithIgnoreList(Ray.new(LocalPlayer.Character.LowerTorso.Position, Vector3.new(0, -LocalPlayer.Character.UpperTorso.Size.y * 4.5, 0)), { LocalPlayer.Character, unpack(require(game.ReplicatedStorage.MainModule).Ignored) })
    ...
    Old = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(self,...)
        if tostring(self.Name) == "MainEvent" then 
            ...
            if pointers["aim_viewer_bypass"]:get() == false and getnamecallmethod() == "FireServer" and args[1] == "UpdateMousePos" and Target ~= nil and pointers["lock_enabled"]:get() and Target.Character and not getgenv().BehindWall and getgenv().InFOVRange then
                if pointers["resolver"]:get() and pointers["resolver_keybind"]:is_active() then 
                    args[2] = Target.Character[HitPart].Position + (ResolverPrediction * pointers["lock_prediction"]:get()) + Vector3.new(0,jump_offset,0)
                else 
                    args[2] = Target.Character[HitPart].Position + (Target.Character.HumanoidRootPart.Velocity *  pointers["lock_prediction"]:get()) + Vector3.new(0,jump_offset,0)
                end 
